...
class Laporan_admin extends Admin_Controller 
{
    ...
    public function __construct()
    {
        ...
        if(!$this->ion_auth->logged_in() or !$this->ion_auth->in_group(['4']))
        {
	...
	{   
        ...
        $data['page_description'] = 'Laporan List';
        $data['message'] = $this->session->flashdata('message');
        $data['pohon_keputusan'] = $this->laporan_model->pohon_keputusan('0', 0);
    ...
    {
        $this->load->library('datatables');
        $data['page_title'] = 'Tabel perhitungan C45';
        $data['page_description'] = 'Laporan List';
        $data['message'] = $this->session->flashdata('message');
        $data['dt_users'] = $this->laporan_model->getAllPerhitungan();
        $this->template->_set_css('admin','dataTables.bootstrap.min.css','adminlte/bower_components/datatables.net-bs/css')
                    ->_set_js('admin','footer','jquery.dataTables.min.js','adminlte/bower_components/datatables.net/js')
                    ->_set_js('admin','footer','dataTables.bootstrap.min.js','adminlte/bower_components/datatables.net-bs/js')
                    // ->_set_js('admin','footer','serverside.dataTables.js','adminlte/script')
                    ...
                    ->_set_js('admin','footer','buttons.html5.min.js','https://cdn.datatables.net/buttons/1.5.2/js', TRUE)
                    ->_set_js('admin','footer','buttons.print.min.js','https://cdn.datatables.net/buttons/1.5.2/js', TRUE)
                    ->_render_admin('index_perhitungan_admin', $data);        
    }


    public function generatePohonC45($idparent, $spasi){
        $root_keputusan = $this->laporan_model->getKeputusan();
        foreach ($root_keputusan as $key => $value) {
            for($i=1;$i<=$spasi;$i++){
                echo "|&nbsp;&nbsp;";
            }
            if ($value->keputusan === 'Delay') {
                $keputusan = "<font color='green'>" . $value->keputusan . "</font>";
            } elseif ($value->keputusan === 'Tidak Delay') {
                $keputusan = "<font color='red'>" .$value->keputusan . "</font>";
            } elseif ($value->keputusan === '?') {
                $keputusan = "<font color='blue'>" .$value->keputusan . "</font>";
            } else {
                $keputusan = "<b>" . $value->keputusan . "</b>";
            }
            echo "<font color='red'>" . $value->atribut . "</font> = " .$value->nilai_atribut . " (Delay = ". $value->jml_delay. ", Tidak Delay = " . $value->jml_tdk_delay . ") : <b> " . $keputusan . "</b><br>";
    ...
    {
        $this->load->library('datatables');
        $data['page_title'] = 'Keputusan C45';
        $data['page_description'] = 'Laporan List';
        $data['message'] = $this->session->flashdata('message');
        $data['pohon_keputusan'] = $this->generatePohonC45('1', 0);
